...
const EditRoomPage = () => {
    ...
    const handleDelete = async () => {
        if(window.confirm('Odayı Silmek İstediğinize Emin Misiniz?')){
            try{
                const result = await ApiService.deleteRoom(roomId);
                if(result.statusCode === 200){
                    setSuccess('Oda Silindi');
                    setTimeout(() => {
                       setSuccess('');
                       navigate('/admin/manage-rooms'); 
                    }, 3000);
    ...
    return(
        <div className="edit-room-container">
            ...
            {succsess && <p className="success-message">{succsess}</p>}
            <div className="edit-room-form">
                ...
                <button className="update-button" onClick={handleUpdate}>Odayı Güncelle</button>
                <button className="delete-button" onClick={handleDelete}>Odayı Sİl</button>
            </div>
        </div>
    );
};
